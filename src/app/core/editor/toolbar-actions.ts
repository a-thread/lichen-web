...
import { BlockTextStyle, SelectionFormatting } from './editor-block.model';
...
export function toggleInline(sel: Selection, marker: string): Selection {
  ...
  return { text: newText, cursor: start + marker.length, selectionEnd: end + marker.length };
}

type BlockType = 'bullet' | 'checklist' | 'numbered';

function currentLine(text: string, cursor: number): { lineStart: number; lineEnd: number; line: string } {
...
export function toggleBlock(sel: Selection, type: BlockType): Selection {
  ...
  const isChecklist = trimmed.startsWith('- [ ] ') || trimmed.startsWith('- [x] ');
...
export function applyTextStyle(sel: Selection, style: BlockTextStyle): Selection {
  const { text } = sel;
  const cursor = sel.cursor;
  const { lineStart, lineEnd, line } = currentLine(text, cursor);
  const stripped = line.replace(/^### /, '').replace(/^## /, '').replace(/^# /, '');

  const prefix = style === 'NORMAL' ? '' : style === 'H1' ? '# ' : style === 'H2' ? '## ' : '### ';
...
function isCursorInsideMarker(text: string, cursor: number, marker: string): boolean {
