...
import { InlineSpan, renderInlineMarkdown } from '../../core/editor/inline-markdown';
...
@Component({
          ...
          @case ('bulletList') {
            <div class="list">
              @for (raw of block.items; track $index) {
                <div class="list-row" [style.paddingLeft.px]="indentLevel(raw) * 16">
          ...
          @case ('numberedList') {
            <div class="list">
              @for (item of block.items; track $index) {
                <div class="list-row" [style.paddingLeft.px]="indentLevel(item.text) * 16">
          ...
          @case ('checklist') {
            <div class="list">
              @for (item of block.items; track item.lineIndex) {
                <div class="checklist-row" [style.paddingLeft.px]="indentLevel(item.text) * 16"
                     (click)="toggleChecklist.emit(item.lineIndex)">
                  <input type="checkbox" [checked]="item.checked" (click)="$event.stopPropagation(); toggleChecklist.emit(item.lineIndex)" />
  ...
  styles: [
    `.preview { display: flex; flex-direction: column; gap: 0.5rem; }
     .heading { font-weight: 600; margin: 0.5rem 0; }
     .heading.h1 { font-size: 1.5rem; }
     .heading.h2 { font-size: 1.25rem; }
     .heading.h3 { font-size: 1.1rem; }
     .text-block { margin: 0; line-height: 1.5; white-space: pre-wrap; }
     code { font-family: monospace; background: rgba(0,0,0,0.06); padding: 0 0.25rem; border-radius: 3px; }
     .list { display: flex; flex-direction: column; gap: 0.4rem; }
     .list-row { display: flex; gap: 0.5rem; }
     .marker { opacity: 0.6; }
     .checklist-row { display: flex; align-items: center; gap: 0.5rem; cursor: pointer; }
     .checklist-row .checked { text-decoration: line-through; opacity: 0.6; }
