...
import { transformEditorInput, TextState } from "./editor-input-transform";
import { toggleChecklistLine } from "../data-access/notes/note-blocks-parser";
import { Selection } from "./toolbar-actions";
...
export class NoteEditorComponent {
  ...
  constructor() {
    ...
    effect(() => {
      ...
      const note = this.notesStore.notes().find((n) => n.id === id);
      if (!note) return;
  ...
  async toggleChecklistReadOnly(lineIndex: number): Promise<void> {
    ...
    await this.notesStore.updateNote(note, this.title(), newText);
    this.existingNote.set(
      this.notesStore.notes().find((n) => n.id === note.id) ?? note,
    );
  }

  hasUnsavedChanges(): boolean {
    return (
      this.title() !== this.originalTitle || this.text() !== this.originalText
    );
  }
  ...
  async save(): Promise<void> {
    ...
    const note = this.existingNote();
    if (note) {
      await this.notesStore.updateNote(note, this.title(), this.text());
      this.existingNote.set(
        this.notesStore.notes().find((n) => n.id === note.id) ?? note,
      );
    } else {
